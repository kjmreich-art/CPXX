...
const App: React.FC = () => {
  ...
  const { currentPhase, elapsedInPhase, phaseIndex } = useMemo(() => {
    ...
    for (let i = 0; i < phases.length; i++) {
      if (secondsElapsedTotal < acc + phases[i].duration) {
        return { currentPhase: phases[i], elapsedInPhase: secondsElapsedTotal - acc, phaseIndex: i };
      }
      acc += phases[i].duration;
    }
    return { currentPhase: phases[phases.length - 1], elapsedInPhase: phases[phases.length - 1].duration, phaseIndex: phases.length - 1 };
  ...
  return (
    <div className="h-screen max-h-screen bg-gradient-to-b from-[#F8FAFC] to-[#FFFFFF] flex flex-col items-center p-4 overflow-hidden touch-none relative">
      ...
      {/* App Header Restored to Previous Design */}
      ...
      <div className="grid grid-cols-2 gap-3 w-full max-w-md mb-2 animate-in fade-in zoom-in-95 duration-700 delay-100">
        <button 
          onClick={() => { audioService.unlock(); setIsRepeat(!isRepeat); }}
          className={`flex flex-col items-center justify-center gap-1 p-3 rounded-[20px] transition-all duration-300 border-2 ${isRepeat ? 'bg-blue-600 border-blue-600 text-white shadow-lg' : 'bg-white border-slate-100 text-slate-500'}`}
        >
        ...
        <button 
          onClick={() => { audioService.unlock(); setShowSettings(true); }}
      ...
      <div className="flex items-center gap-4 mt-4 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-300">
        ...
        <button
          ...
          className={`group flex items-center justify-center gap-3 px-8 py-4 rounded-[28px] font-black text-lg transition-all duration-300 active:scale-95 ${isActive ? 'bg-slate-800 text-white shadow-xl' : 'bg-blue-600 text-white shadow-xl shadow-blue-100'}`}
        >
      ...
      <div className="flex flex-wrap justify-center gap-1.5 mt-4 mb-2 animate-in fade-in duration-700 delay-400">
        {phases.map((p, i) => (
          <div 
            key={p.id}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black transition-all duration-500 ${phaseIndex === i ? `${p.color} ring-2 ring-offset-1 ring-blue-50 scale-105 shadow-sm` : 'bg-white text-slate-300 border border-slate-100 opacity-60'}`}
          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-[40px] w-full max-w-sm p-8 shadow-2xl border border-white/20">
            <div className="flex items-center gap-4 mb-6">
              <div className="p-2 bg-blue-50 rounded-xl text-blue-600"><Settings2 className="w-5 h-5" /></div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-3">
                {[480, 540, 600, 630, 660, 720].map((sec) => (
                  <button
                    key={sec}
                    onClick={() => { audioService.unlock(); setConsultationDuration(sec); }}
                    className={`py-4 rounded-2xl font-black text-sm border-2 transition-all duration-300 ${consultationDuration === sec ? 'bg-blue-600 border-blue-600 text-white shadow-md' : 'bg-slate-50 border-transparent text-slate-400'}`}
                  >
              ...
              <button
                onClick={() => { audioService.unlock(); setShowSettings(false); }}
